...
// Mock shifts — replaced with Supabase later
const mockShifts = [
    ...
    <div className="px-4 py-4 space-y-4">
      ...
      <div className="flex gap-2">
        <div className="flex-1 relative">
          <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2" style={{ color: "rgba(255,255,255,0.25)" }} />
        ...
        <button onClick={() => setShowFilters(!showFilters)}
          ...
          style={{ background: showFilters ? "rgba(239,71,111,0.2)" : "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.08)" }}>
          {showFilters ? <X size={18} style={{ color: "#EF476F" }} /> : <SlidersHorizontal size={18} style={{ color: "rgba(255,255,255,0.4)" }} />}
        </button>
      </div>
      ...
      <div className="flex gap-2 overflow-x-auto pb-1 -mx-4 px-4 scrollbar-hide">
        {sectorFilters.map((f) => (
          <button key={f.value} onClick={() => setSector(f.value)}
            ...
            style={{
              background: sector === f.value ? "#EF476F" : "rgba(255,255,255,0.04)",
              color: sector === f.value ? "white" : "rgba(255,255,255,0.4)",
              border: `1px solid ${sector === f.value ? "#EF476F" : "rgba(255,255,255,0.06)"}`,
            }}>
            {f.label}
          </button>
        ))}
      ...
      <div className="text-xs font-medium" style={{ color: "rgba(255,255,255,0.3)" }}>
        {filtered.length} shifts gevonden
      </div>
              ...
              <Link href={`/shifts/${shift.id}`}>
                <motion.div whileTap={{ scale: 0.98 }}
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <h3 className="text-white font-bold">{shift.title}</h3>
                      <p className="text-xs" style={{ color: "rgba(255,255,255,0.35)" }}>{shift.company}</p>
                    </div>
                  ...
                  <div className="mt-3 flex items-center gap-2">
                    <div className="flex-1 h-1.5 rounded-full" style={{ background: "rgba(255,255,255,0.06)" }}>
                      <div className="h-full rounded-full transition-all"
                        style={{
                          width: `${(shift.workers_filled / shift.workers_needed) * 100}%`,
                          background: shift.workers_filled >= shift.workers_needed ? "#A7DADC" : "#EF476F",
                        }} />
                    </div>
                    <span className="text-[10px] font-medium" style={{ color: "rgba(255,255,255,0.3)" }}>
                      {shift.workers_filled}/{shift.workers_needed}
                    </span>
                  </div>
                </motion.div>
              </Link>
