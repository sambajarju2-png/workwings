...
export default function LoginPage() {
  ...
  async function handleGoogleLogin() {
    ...
    await supabase.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: `${window.location.origin}/callback?type=${tab}` },
    });
  }
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setLoading(false);
    router.push(tab === "company" ? "/admin" : "/shifts");
  }
      ...
      <div className="flex-1 px-4 pb-8 max-w-sm mx-auto w-full">
        <div className="flex rounded-xl p-1 mb-6 bg-background-alt border border-border">
          {(["worker", "company"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              ...
              style={{ background: tab === t ? "var(--color-surface)" : "transparent", color: tab === t ? "var(--color-foreground)" : "var(--color-foreground-subtle)" }}>
        ...
        {tab === "worker" && (
          <>
            ...
            <div className="flex items-center gap-3 mb-4"><div className="flex-1 h-px bg-border" /><span className="text-xs text-foreground-subtle">of</span><div className="flex-1 h-px bg-border" /></div>
          </>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <motion.button type="submit" disabled={loading} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
            ...
            {loading ? <Loader2 size={18} className="animate-spin" /> : <>Inloggen <ArrowRight size={16} /></>}
          </motion.button>
        </form>

        <p className="text-center text-sm text-foreground-subtle mt-6">
          Nog geen account? <Link href="/signup" className="font-bold" style={{ color: "#EF476F" }}>Aanmelden</Link>
        </p>
      </div>
    </div>
  );
