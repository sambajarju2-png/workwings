...
export default function CompanySignupPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/auth/register-company", {
      ...
      body: JSON.stringify({ email, password, companyName, kvk, address, city, postalCode, sectors: selectedSectors }),
    });
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <div className="flex gap-2">
              <div className="relative flex-1"><Hash size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-foreground-subtle" /><input value={kvk} onChange={e => { setKvk(e.target.value); setKvkFound(false); }} placeholder="12345678" className={inputIcon} /></div>
              <motion.button type="button" onClick={lookupKvk} disabled={kvk.length < 8 || kvkLoading} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} className="px-4 rounded-xl font-semibold text-sm text-white disabled:opacity-50 flex items-center gap-2" style={{ background: "#023047" }}>
                {kvkLoading ? <Loader2 size={14} className="animate-spin" /> : <Search size={14} />} Zoek
              </motion.button>
            </div>
            {kvkFound && <p className="text-xs mt-1 flex items-center gap-1" style={{ color: "#0e8a8d" }}><CheckCircle size={12} /> Gevonden</p>}
          </div>
