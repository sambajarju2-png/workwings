...
import { ArrowRight, ArrowLeft, Loader2, CheckCircle, User, MapPin, Mail, Lock } from "lucide-react";
...
export default function WorkerSignupPage() {
  ...
  async function handleGoogleSignup() {
    ...
    if (!supabase) { setError("Niet geconfigureerd"); return; }
    ...
    <div className="min-h-screen bg-background px-4 py-8">
      <div className="max-w-sm mx-auto">
        ...
        <div className="flex gap-2 mb-8">
          {[1, 2, 3].map(s => (
            <div key={s} className="flex-1 h-1.5 rounded-full" style={{ background: step >= s ? "#EF476F" : "var(--color-border)" }} />
          ))}
        </div>

        <h1 className="text-2xl font-black text-foreground mb-2">
          {step === 1 ? "Account aanmaken" : step === 2 ? "Over jou" : "Jouw sectoren"}
        </h1>
        <p className="text-sm text-foreground-subtle mb-8">
          {step === 1 ? "Maak je WorkWings account aan" : step === 2 ? "Vertel ons wie je bent" : "In welke sectoren wil je werken?"}
        </p>
        ...
        {step === 1 && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
            <button onClick={handleGoogleSignup}
              className="w-full py-3.5 rounded-xl font-semibold text-sm flex items-center justify-center gap-3 bg-surface border border-border text-foreground">
            ...
            <div className="flex items-center gap-3"><div className="flex-1 h-px bg-border" /><span className="text-xs text-foreground-subtle">of</span><div className="flex-1 h-px bg-border" /></div>
        ...
        {step === 2 && (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-4">
            ...
            <input value={phone} onChange={e => setPhone(e.target.value)} placeholder="Telefoonnummer (optioneel)"
              className="w-full px-4 py-3.5 rounded-xl text-sm text-foreground bg-surface border border-border outline-none" />
            <motion.button onClick={handleStep2} disabled={loading || !firstName || !lastName || !city} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
              className="w-full py-3.5 rounded-xl text-white font-bold flex items-center justify-center gap-2 disabled:opacity-50"
              style={{ background: "linear-gradient(135deg, #EF476F, #D93A5E)" }}>
              {loading ? <Loader2 size={18} className="animate-spin" /> : <>Doorgaan <ArrowRight size={16} /></>}
            </motion.button>
            ...
            <div className="grid grid-cols-2 gap-3">
              {sectors.map(s => (
                <button key={s.value} onClick={() => toggleSector(s.value)}
                  ...
                  style={{
                    background: selectedSectors.includes(s.value) ? "rgba(239,71,111,0.06)" : "var(--color-surface)",
