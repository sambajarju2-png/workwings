    ...
    <section id="branches" className="py-24 px-4 bg-background">
      <div className="max-w-6xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          ...
          <h2 className="text-4xl md:text-5xl font-black mt-4 text-foreground">Shifts in <span style={{ color: "#EF476F" }}>elke branche</span></h2>
        </motion.div>
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          {sectors.map((s, i) => (
            <motion.div key={i} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }}
              transition={{ delay: i * 0.08 }} whileHover={{ scale: 1.05, y: -4 }} whileTap={{ scale: 0.98 }}
              className="relative p-6 rounded-2xl text-white cursor-pointer overflow-hidden group" style={{ background: s.bg }}>
              <div className="absolute top-0 right-0 w-24 h-24 opacity-10" style={{ background: "radial-gradient(circle, white, transparent)" }} />
